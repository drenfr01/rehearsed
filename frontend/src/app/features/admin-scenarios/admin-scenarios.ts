...
@Component({
  ...
  templateUrl: './admin-scenarios.html',
  styleUrl: './admin-scenarios.css',
})
export class AdminScenarios implements OnInit {
  ...
  loadScenarios() {
    ...
    const subscription = this.adminService.getAllScenarios().subscribe({
      next: (scenarios) => {
  ...
  createScenario() {
    if (this.createScenarioForm.valid) {
      ...
      const subscription = this.adminService.createScenario(scenarioData).subscribe({
        next: (scenario) => {
  ...
  deleteScenario(scenarioId: number, name: string) {
    if (confirm(`Are you sure you want to delete scenario "${name}"?`)) {
      const subscription = this.adminService.deleteScenario(scenarioId).subscribe({
        next: () => {
