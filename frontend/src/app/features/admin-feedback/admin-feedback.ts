...
export class AdminFeedback implements OnInit {
  ...
  loadData() {
    ...
    const subscription = this.adminService.getAllFeedback().subscribe({
      next: (feedbacks) => {
  ...
  createFeedback() {
    if (this.createFeedbackForm.valid) {
      ...
      const subscription = this.adminService.createFeedback(feedbackData).subscribe({
        next: (feedback) => {
  ...
  private saveEdit(feedbackId: number, data: EditFeedbackDialogResult) {
    const subscription = this.adminService.updateFeedback(feedbackId, data).subscribe({
  ...
  deleteFeedback(feedbackId: number, feedbackType: FeedbackType) {
    if (confirm(`Are you sure you want to delete the "${feedbackType}" feedback?`)) {
      const subscription = this.adminService.deleteFeedback(feedbackId).subscribe({
        next: () => {
