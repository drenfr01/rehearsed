...
@Component({
  ...
  templateUrl: './admin-agent-personalities.html',
  styleUrl: './admin-agent-personalities.css',
})
export class AdminAgentPersonalities implements OnInit {
  ...
  loadPersonalities() {
    ...
    const subscription = this.adminService.getAllAgentPersonalities().subscribe({
      next: (personalities) => {
  ...
  createPersonality() {
    if (this.createPersonalityForm.valid) {
      ...
      const subscription = this.adminService.createAgentPersonality(personalityData).subscribe({
        next: (personality) => {
  ...
  deletePersonality(personalityId: number, name: string) {
    if (confirm(`Are you sure you want to delete agent personality "${name}"?`)) {
      const subscription = this.adminService.deleteAgentPersonality(personalityId).subscribe({
        next: () => {
