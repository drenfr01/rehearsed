...
async def list_my_scenarios(request: Request, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        scenarios = await db_service.get_user_local_scenarios(user.id)
        return [
            ScenarioAdminResponse(
                id=s.id,
                name=s.name,
...
async def create_scenario(
    ...
    try:
        ...
        scenario = await db_service.create_user_scenario(
            user_id=user.id,
            name=name,
...
async def update_scenario(
    ...
    try:
        ...
        updated_scenario = await db_service.update_user_scenario(
...
async def delete_scenario(request: Request, scenario_id: int, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        await db_service.delete_user_scenario(scenario_id, user.id)
...
async def copy_scenario_to_local(
    ...
    """
    try:
        new_scenario = await db_service.copy_scenario_to_user(scenario_id, user.id, copy_agents=True)
...
async def list_my_agent_personalities(request: Request, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        personalities = await db_service.get_user_local_agent_personalities(user.id)
        return [
            AgentPersonalityResponse(
                id=p.id,
                name=p.name,
...
async def create_agent_personality(
    ...
    try:
        ...
        personality = await db_service.create_user_agent_personality(
            user_id=user.id,
            name=name,
...
async def update_agent_personality(
    ...
    try:
        ...
        updated_personality = await db_service.update_user_agent_personality(
...
async def delete_agent_personality(
    ...
    """
    try:
        await db_service.delete_user_agent_personality(personality_id, user.id)
...
async def copy_agent_personality_to_local(
    ...
    """
    try:
        new_personality = await db_service.copy_agent_personality_to_user(personality_id, user.id)
...
async def list_my_agents(request: Request, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        agents = await db_service.get_user_local_agents(user.id)
        return [
            AgentResponse(
                id=a.id,
                name=a.name,
...
async def create_agent(
    ...
    try:
        ...
        agent = await db_service.create_user_agent(
            user_id=user.id,
            agent_id=agent_id,
            name=name,
...
async def update_agent(
    ...
    try:
        ...
        old_agent = await db_service.get_agent(agent_id)
        old_scenario_id = old_agent.scenario_id if old_agent else None

        updated_agent = await db_service.update_user_agent(
            agent_id=agent_id,
            user_id=user.id,
            name=name,
            voice=voice,
...
async def delete_agent(request: Request, agent_id: str, user: User = Depends(get_current_user_from_session)):
    ...
    try:
        ...
        await db_service.delete_user_agent(agent_id, user.id)
...
async def copy_agent_to_local(
    ...
    try:
        ...
        scenario = await db_service.get_scenario(target_scenario_id)
        if not scenario:
        ...
        new_agent = await db_service.copy_agent_to_user(agent_id, user.id, target_scenario_id)
...
async def list_my_feedback(request: Request, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        feedbacks = await db_service.get_user_local_feedback(user.id)
        return [
            FeedbackResponse(
                id=f.id,
...
async def create_feedback(
    ...
    """
    try:
        feedback = await db_service.create_user_feedback(
            user_id=user.id,
...
async def update_feedback(
    ...
    """
    try:
        updated_feedback = await db_service.update_user_feedback(
...
async def delete_feedback(request: Request, feedback_id: int, user: User = Depends(get_current_user_from_session)):
    ...
    """
    try:
        await db_service.delete_user_feedback(feedback_id, user.id)
...
async def copy_feedback_to_local(
    ...
    """
    try:
        new_feedback = await db_service.copy_feedback_to_user(feedback_id, user.id)
