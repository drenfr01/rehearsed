...
@observe(name="gemini_live_session")
async def _handle_gemini_session(
    websocket: WebSocket,
    session_id: str,
    user_id: int,
    db: DatabaseService,
):
    """Run the Gemini Live session loop with Langfuse tracing.

    The @observe decorator creates a top-level Langfuse trace, matching the
    pattern used in the chatbot's get_response flow.
    """
    langfuse = get_client()
            ...
            while True:
                ...
                if msg_type == "setup":
                    ...
                    async def _relay_from_gemini(_session=gemini_session):
                        ...
                        Note: _session is captured via default arg to avoid closure
                        over a mutable outer variable.
                        """
...
async def gemini_live_ws(
    ...
    await _handle_gemini_session(websocket, session_id, user_id, db)
